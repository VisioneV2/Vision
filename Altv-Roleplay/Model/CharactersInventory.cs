...
namespace Altv_Roleplay.Model
{
    class CharactersInventory
    {
        ...
        public static bool ExistCharacterItem(int charId, string itemName, string itemLocation)
        {
            try
            {
                var item = CharactersInventory_.ToList().FirstOrDefault(i => i.charId == charId && i.itemName.TrimEnd() == itemName.TrimEnd() && i.itemLocation == itemLocation);
        ...
        public static int GetCharacterItemAmount(int charId, string itemName, string itemLocation)
        {
            try
            {
                if (charId == null || itemName == null || itemLocation == null)
                {
                    return 0;
                }
        ...
        public static float GetCharacterItemWeight(int charId, string itemLocation)
        {
            try
            {
                if (charId == null || itemLocation == null || charId <= 0 || string.IsNullOrWhiteSpace(itemLocation)) return 0f;
