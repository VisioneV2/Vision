...
namespace Altv_Roleplay.Model
{
    class ServerFactions
    {
        ...
        public static List<Server_Faction_Waffen_Labor_Items> ServerFactionWaffenLaborItems_ = new List<Server_Faction_Waffen_Labor_Items>();
        ...
        public static bool IsFraktionslagerLocked(int factionId)
        {
            Server_Factions x = ServerFactions_.ToList().FirstOrDefault(y => y.id == factionId);
            if (x != null) return x.isFraktionslagerLocked;
            return false;
        }
        ...
        public static bool IsWaffenLaborLocked(int factionId)
        {
            Server_Factions x = ServerFactions_.ToList().FirstOrDefault(y => y.id == factionId);
            if (x != null) return x.isWaffenLaborLocked;
            return false;
        }
        ...
        public static string GetWaffenLaborItemsFromPlayer(int factionId, int accId)
        {
            return System.Text.Json.JsonSerializer.Serialize(ServerFactionWaffenLaborItems_.ToList().Where(x => x.factionId == factionId && x.accountId == accId).Select(x => new
            {
                x.itemName,
                x.itemAmount,
        ...
        public static int GetWaffenLaborItemAmount(int factionId, int accountId, string itemName)
        {
            Server_Faction_Waffen_Labor_Items x = ServerFactionWaffenLaborItems_.ToList().FirstOrDefault(y => y.factionId == factionId && y.accountId == accountId && y.itemName == itemName);
            if (x != null) return x.itemAmount;
            return 0;
        }

        public static void AddWaffenLaborItem(int factionId, int accountId, string itemName, int itemAmount)
        {
            try
            {
                if (factionId <= 0 || accountId <= 0 || string.IsNullOrWhiteSpace(itemName) || itemAmount <= 0) return;
                Server_Faction_Waffen_Labor_Items existItem = ServerFactionWaffenLaborItems_.FirstOrDefault(x => x.factionId == factionId && x.accountId == accountId && x.itemName == itemName);
                if (existItem != null)
                {
                    existItem.itemAmount += itemAmount;
                    using (var db = new gtaContext())
                    {
                        db.Server_Faction_Waffen_Labor_Items.Update(existItem);
                        db.SaveChanges();
                    }
                }
                else
                {
                    Server_Faction_Waffen_Labor_Items newItem = new Server_Faction_Waffen_Labor_Items
                    {
                        factionId = factionId,
                        accountId = accountId,
                        itemAmount = itemAmount,
                    ...
                    ServerFactionWaffenLaborItems_.Add(newItem);
                    using (var db = new gtaContext())
                    {
                        db.Server_Faction_Waffen_Labor_Items.Add(newItem);
                        db.SaveChanges();
                    }
                }
            }
        ...
        public static void RemoveWaffenLaborItemAmount(int factionId, int accountId, string itemName, int itemAmount)
        {
            try
            {
                if (factionId <= 0 || accountId <= 0 || string.IsNullOrWhiteSpace(itemName) || itemAmount <= 0) return;
                Server_Faction_Waffen_Labor_Items item = ServerFactionWaffenLaborItems_.FirstOrDefault(x => x.factionId == factionId && x.accountId == accountId && x.itemName == itemName);
                if (item == null) return;
                item.itemAmount -= itemAmount;
                if (item.itemAmount > 0)
                {
                    using (var db = new gtaContext())
                    {
                        db.Server_Faction_Waffen_Labor_Items.Update(item);
                        db.SaveChanges();
                    }
                }
                else
                {
                    using (var db = new gtaContext())
                    {
                        db.Server_Faction_Waffen_Labor_Items.Remove(item);
                        db.SaveChanges();
                    }
                    ServerFactionWaffenLaborItems_.Remove(item);
                }
            }
            catch (Exception e)
            {
        ...
        public static Position GetFraktionslagerExitPosition(int factionId)
        {
            Server_Factions x = ServerFactions_.ToList().FirstOrDefault(y => y.id == factionId);
            if (x != null)
            {
        ...
        public static Position GetWaffenLaborExitPosition(int factionId)
        {
            Server_Factions x = ServerFactions_.ToList().FirstOrDefault(y => y.id == factionId);
            if (x != null)
            {
        ...
        public static void SetFraktionslagerLocked(int factionId, bool state)
        {
            Server_Factions x = ServerFactions_.FirstOrDefault(y => y.id == factionId);
            if (x == null) return;
            x.isFraktionslagerLocked = state;
        }
        ...
        public static void SetWaffenLaborLocked(int factionId, bool state)
        {
            Server_Factions x = ServerFactions_.FirstOrDefault(y => y.id == factionId);
            if (x == null) return;
            x.isWaffenLaborLocked = state;
        }
        #endregion
