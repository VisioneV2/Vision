    ...
    class InventoryHandler : IScript
    {
        ...
        public async Task UseItem(ClassicPlayer player, string itemname, int itemAmount, string fromContainer)
        {
            try
            {
                ...
                else if (itemname == "Agentschutzweste")
                {

                    player.SetPlayerIsCuffed("handcuffs", true);
                    CharactersInventory.RemoveCharacterItemAmount(charId, "Agentschutzweste", 1, fromContainer);
                    Characters.SetCharacterArmor(charId, 100);
                    player.Armor = 100;
                    if (Characters.GetCharacterGender(charId)) player.EmitLocked("Client:SpawnArea:setCharClothes", 9, 17, 2);
                ...
                else if (itemname == "Verband")
                {
                    ...
                    InventoryAnimation(player, "verband", 0);
                    player.EmitLocked("Client:HUD:UpdateDesire", Characters.GetCharacterArmor(charId), Characters.GetCharacterHealth(charId), Characters.GetCharacterHunger(charId), Characters.GetCharacterThirst(charId)); //HUD updaten
                }
                ...
                else if (itemname == "Schutzweste")
                {
                    ...
                    player.Armor = 100;
                    if (Characters.GetCharacterGender(charId)) player.EmitLocked("Client:SpawnArea:setCharClothes", 9, 17, 2);
                    else player.EmitLocked("Client:SpawnArea:setCharClothes", 9, 15, 2); // Schutzweste
                    await Task.Delay(5000);
                    player.SetPlayerIsCuffed("handcuffs", false);
                ...
                else if (itemname == "Verbandskasten")
                {
                    ...
                    player.SetPlayerIsCuffed("handcuffs", true);
                    player.GiveWeapon(WeaponModel.Fist, 0, true);
                    InventoryAnimation(player, "verband", 5000);
                    ...
                    player.Health = 200;
                    HUDHandler.SendNotification(player, 2, 5000, $"{itemname} Benuzt{itemAmount}");
        ...
        public async Task GiveItem(ClassicPlayer player, string itemname, int itemAmount, string fromContainer, int targetPlayerId)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (invWeight + itemWeight <= 15f)
                    {
                        DiscordLog.SendEmbed("itemlog", "Itemgeben Log", $"{ Characters.GetCharacterName(User.GetPlayerOnline(player))} hat {Characters.GetCharacterName(User.GetPlayerOnline(targetPlayer))} den Gegenstand {itemname} {itemAmount}x gegeben.");
                        if (itemname.Contains("Bargeld")) DiscordLog.SendEmbed("geldlog", "Geld geben Log", $"{ Characters.GetCharacterName(User.GetPlayerOnline(player))} hat {Characters.GetCharacterName(User.GetPlayerOnline(targetPlayer))} den Gegenstand {itemname} {itemAmount}x gegeben.");
                        HUDHandler.SendNotification(targetPlayer, 1, 5000, $"Eine Person hat dir den Gegenstand {itemname} ({itemAmount}x) gegeben.");
                        HUDHandler.SendNotification(player, 2, 5000, $"Du hast einem Spieler den Gegenstand {itemname} ({itemAmount}x) gegeben.");
                        CharactersInventory.AddCharacterItem(targetPlayerId, itemname, itemAmount, "inventory");
                        CharactersInventory.RemoveCharacterItemAmount(charId, itemname, itemAmount, fromContainer);
        ...
        internal static void InventoryAnimation(IPlayer player, string Animation, int duration)
        {
            ...
            else if (Animation == "farmPickup") player.EmitLocked("Client:Inventory:PlayAnimation", "amb@world_human_gardener_plant@male@base", "base", duration, 1, false);
            ...
            else if (Animation == "weste") player.EmitLocked("Client:Inventory:PlayAnimation", "amb@prop_human_bum_bin@base", "base", duration, 1, false);
            ...
            else if (Animation == "verband") player.EmitLocked("Client:Inventory:PlayAnimation", "amb@medic@standing@kneel@base", "base", duration, 1, false);
            //Sheytan
            else if (Animation == "repair") player.EmitLocked("Client:Inventory:PlayAnimation", "mini@repair", "fixing_a_ped", 3000, 1, false);
        }
