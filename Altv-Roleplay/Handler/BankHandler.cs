...
namespace Altv_Roleplay.Handler
{
    class BankHandler : IScript
    {
        ...
        public async Task TransferATMMoney(IPlayer player, int accountNumber, int targetNumber, int amount, string zoneName)
        {
            ...
            if (!CharactersBank.ExistBankAccountNumber(targetNumber)) { HUDHandler.SendNotification(player, 3, 5000, $"Die angegebene Kontonummer existiert nicht!"); return; }
        ...
        public async Task DepositFactionMoney(IPlayer player, string type, int factionId, int moneyAmount) //Type: faction | company
        {
            try
            {
                ...
                if (type == "faction")
                {
                    DiscordLog.SendEmbed("frakbank", "Fraktionsbank LOG", $"{Characters.GetCharacterName(charid)} hat {moneyAmount} auf das Fraktionskonto (ID: {factionId}) eingezahlt.");
                ...
                {
                    DiscordLog.SendEmbed("frakbank", "Unternehmenskonto LOG", $"{Characters.GetCharacterName(charid)} hat {moneyAmount} auf das Unternehmenskonto (ID: {factionId}) eingezahlt.");
        ...
        public async Task WithdrawFactionMoney(IPlayer player, string type, int factionId, int moneyAmount) //Type: faction | company
        {
            try
            {
                ...
                else if (type == "company")
                {
                    ...
                    DiscordLog.SendEmbed("frakbank", "Unternehmenskonto Log", $"{Characters.GetCharacterName(charid)} hat {moneyAmount}vom Unternehmenskonto (ID: {factionId}) abgebucht.");
