...
namespace Altv_Roleplay.Handler
{
    class SmartphoneHandler : IScript
    {
        ...
        public void requestPhonebusiness(IPlayer player)
        {
            try
            {
        ...
        public void requestPhoneVerlauf(IPlayer player)
        {
            try
            {
                if (player == null || !player.Exists || player.GetCharacterMetaId() <= 0) return;
...

        {
            ...
            {
                if (player == null || !player.Exists || player.CharacterId <= 0) return;
                if (!ServerFactions.IsCharacterInAnyFaction(player.CharacterId) || (ServerFactions.GetCharacterFactionId(player.CharacterId) != 1 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 2 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 3 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 8) || !ServerFactions.IsCharacterInFactionDuty(player.CharacterId))
                {
...

        {
            try
            {
                if (player == null || !player.Exists || player.CharacterId <= 0 || !ServerFactions.IsCharacterInAnyFaction(player.CharacterId) || !ServerFactions.IsCharacterInFactionDuty(player.CharacterId) || (ServerFactions.GetCharacterFactionId(player.CharacterId) != 1 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 2 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 3 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 8)) return;
        ...
        public void joinRadioFrequence(ClassicPlayer player, string radioFrequence)
        {
            try
            {
                ...
                float fr = float.Parse(radioFrequence);


                if (fr < 11000) // falls die frequenz im verschlüsseltem bereich liegt
                {
                    int frequencyFactionId = 0; // faction id des verschlüsselten chats
                    ...
                    {
                        first = Convert.ToInt32(fr.ToString().Substring(0, 1));
                    }

                    switch (first) // gibt die factionId für den funk wieder
                    {
                        case 1: // LSPD Funk
                            frequencyFactionId = 1;
                            break;
                        case 2: // FIB Funk
                            frequencyFactionId = 3;
                            break;
                        case 3: // DOJ Funk
                            frequencyFactionId = 2;
                            break;
                        case 4: // LSMD Funk
                            frequencyFactionId = 4;
                            break;
                        case 5: // ACLS Funk
                            frequencyFactionId = 5;
                            break;
                        case 6: // Fahrschule Funk
                            frequencyFactionId = 6;
                            break;
                        case 8: // Government Funk
                            frequencyFactionId = 8;
                            break;
                        case 10: // Staatsfrak Funk
                            frequencyFactionId = -1;
                            break;
                    }

                    if (fr < 11000) // funkfrequenz unter 10.00 (staatsfraks)
                    {
                    ...
                    if (frequencyFactionId == -1) // falls es sich um den allgemeinen Staatsfrak Funk handelt
                    {
                        if (!ServerFactions.IsCharacterInAnyFaction(player.CharacterId) || !(ServerFactions.GetCharacterFactionId(player.CharacterId) < 10) || !ServerFactions.IsCharacterInFactionDuty(player.CharacterId)) // Checkt ob man in einer Staatsfrak ist (für allgemeinen Staatsfunk)
                        {
                            HUDHandler.SendNotification(player, 4, 5000, "Dieser Funk ist Verschlüsselt!");
                            Characters.SetCharacterCurrentFunkFrequence(player.CharacterId, null);
                            player.EmitLocked("Client:Smartphone:setCurrentFunkFrequence", "null");
                    ...
                    {
                        if (!ServerFactions.IsCharacterInAnyFaction(player.CharacterId) || (ServerFactions.GetCharacterFactionId(player.CharacterId) != frequencyFactionId) || !ServerFactions.IsCharacterInFactionDuty(player.CharacterId)) // Checkt ob man in einer Staatsfrak ist (für allgemeinen Staatsfunk)
                        {
                            HUDHandler.SendNotification(player, 4, 5000, "Dieser Funk ist Verschlüsselt!");
                            Characters.SetCharacterCurrentFunkFrequence(player.CharacterId, null);
                            player.EmitLocked("Client:Smartphone:setCurrentFunkFrequence", "null");
