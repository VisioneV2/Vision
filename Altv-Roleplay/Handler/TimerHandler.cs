...
namespace Altv_Roleplay.Handler
{
    class TimerHandler
    {
        ...
        //public static async void banTimer(object sender, ElapsedEventArgs e)
        //{
        //    foreach (IPlayer player in Alt.GetAllPlayers().ToList())
        //    {
        //        var loginInfo = LoginHandler.MakePostRequest("https://endlessrp.eu/WbbVerify.php", User.GetPlayerUsername(User.GetPlayerOnline(player)), User.GetPlayerPassword(User.GetPlayerUsername(User.GetPlayerOnline(player))), "VEwSvcqG1N0SwcxayaBdSvnNbX4gbESh7lsnizVVTm5SLoHInQt54wS9lurd5Fr4EHKYhHfgJY6X3LVzQhaBr2ARXf2iedGRDOLVqi7eoRUWiV3dgH6R9cDe3siXiBht").Result;
        //        if (loginInfo.UserData.Banned)
        //        {
        //            if (User.IsPlayerBanned(player))
        //            {
        //                player.EmitLocked("Client:Login:showError", $"Du wurdest gebannt. Grund: {loginInfo.UserData.BanReason}", loginInfo.UserData.BanReason);
        //                player.Kick($"Du wurdest gebannt. Grund: {loginInfo.UserData.BanReason}");
        //            }
        //            else
        //            {
        //                User.SetPlayerBanned(player, true, loginInfo.UserData.BanReason);
        //                player.EmitLocked("Client:Login:showError", "Du wurdest gebannt. Grund: {0}", loginInfo.UserData.BanReason);
        //                player.Kick($"Du wurdest gebannt. Grund: {loginInfo.UserData.BanReason}");
        //            }
        ...
        //    }
        //}
        public static void OnFuelTimer(object sender, ElapsedEventArgs e)
            ...
            try
            {
                ...
                foreach (IVehicle Veh in Alt.GetAllVehicles().ToList())
                {
                    ...
                    using (var vRef = new VehicleRef(Veh))
                    {
                        ...
                        {
                            if (Veh == null || !Veh.Exists) continue;
                            if (Veh.EngineOn == true) { ServerVehicles.SetVehicleFuel(Veh, ServerVehicles.GetVehicleFuel(Veh) - 0.125f); } // kraftstoff verbrauch
                            ServerVehicles.SaveVehiclePositionAndStates(Veh);
                            ServerVehicles.SaveVehiclePositionAndStates(Veh);
                            ServerVehicles.SaveVehiclePositionAndStates(Veh);
                            ServerVehicles.SaveVehiclePositionAndStates(Veh);
                            if (Veh.BodyHealth < 100)
                            {
                ...
                stopwatch.Stop();
                Alt.Log($"KM-State und Client:HUD:UpdateDesire took: {stopwatch.Elapsed}");
            }
            catch (Exception ex)
            {
                Alt.Log($"{ex}");
        ...
        public static void OnKilometerTimer(object sender, ElapsedEventArgs e)
...

        {
            try
            {
                ...
                foreach (IPlayer player in Alt.GetAllPlayers().ToList())
                {
                    ...
                    {
                        if (!playerReference.Exists) return;
                        if (player == null || !player.Exists || !((ClassicPlayer)player).isSpawned || ((ClassicPlayer)player).IsBlocked) continue;
                        lock (player)
                        {
                            if (player == null || !player.Exists) continue;
                            int charId2 = User.GetPlayerOnline(player);
                        ...
                        int charId = User.GetPlayerOnline(player);
                        player.EmitLocked("Client:HUD:UpdateDesire", Characters.GetCharacterArmor(charId), Characters.GetCharacterHealth(charId), Characters.GetCharacterHunger(charId), Characters.GetCharacterThirst(charId)); //HUD updaten
                        player.Emit("Client:HUD:updateMoney", CharactersInventory.GetCharacterItemAmount(User.GetPlayerOnline(player), "Bargeld", "inventory"));
                    }
                }
                stopwatch.Stop();
                Alt.Log($"OnEntityTimer: KILOMETER AMK Foreach benötigte: {stopwatch.Elapsed}");
...
*//*                        }
                ...
                if (player == null || Characters.IsCharacterAnimal(((ClassicPlayer)player).CharacterId)) continue; using (var pRef = new PlayerRef(player))
                {
                    ...
                    lock (player)
                    {
                        if (player.Exists && User.GetPlayerOnline(player) != 0)
                        {
                            ...
                            {
                                HUDHandler.SendNotification(player, 1, 5000, $"Du solltest einen Arzt besuchen!");
                            }
        ...
        internal static void WaffenLaborTimer(Object sender, ElapsedEventArgs a)
        {
            try
            {
                ...
                foreach (var faction in ServerFactions.ServerFactions_.ToList())
                {
                    ...
                    {
                        if (ServerFactions.GetWaffenLaborItemAmount(factionMember.factionId, factionMember.charId, "Eisenbarren") >= 10 && ServerFactions.GetWaffenLaborItemAmount(factionMember.factionId, factionMember.charId, "Zinkbarren") >= 10 && ServerFactions.GetWaffenLaborItemAmount(factionMember.factionId, factionMember.charId, "Plastik") >= 10)
                        {
                            ServerFactions.RemoveWaffenLaborItemAmount(factionMember.factionId, factionMember.charId, "Eisenbarren", 10);
                            ServerFactions.RemoveWaffenLaborItemAmount(factionMember.factionId, factionMember.charId, "Zinkbarren", 10);
                            ServerFactions.RemoveWaffenLaborItemAmount(factionMember.factionId, factionMember.charId, "Plastik", 5);
                            ServerFactions.AddWaffenLaborItem(factionMember.factionId, factionMember.charId, "Waffenteile", 1);
                        }
                    }
                }
            }
