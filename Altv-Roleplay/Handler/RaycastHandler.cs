    ...
    class RaycastHandler : IScript
    {
        ...
        public async Task GetMenuPlayerItems(IPlayer player, string type, IPlayer targetPlayer)
        {
            try
            {
                ...
                {
                    interactHTML += "<li class='interactitem' id='InteractionMenu-playerGiveTakeHandcuffs' data-action='playerGiveTakeHandcuffs' data-actionstring='Handschellen an/ablegen'><img src='../utils/img/Handschellen.png'></li>";
                }
                ...
                if (ServerFactions.IsCharacterInAnyFaction(charId) && ServerFactions.IsCharacterInFactionDuty(charId))
                {
                    ...
                    if ((ServerFactions.GetCharacterFactionId(charId) == 1 || ServerFactions.GetCharacterFactionId(charId) == 3 || ServerFactions.GetCharacterFactionId(charId) == 8) && player.Position.IsInRange(Constants.Positions.Arrest_Position, 5f) && targetPlayer.Position.IsInRange(Constants.Positions.Arrest_Position, 5f))
                    {
                    ...
                    {
                        interactHTML += "<li class='interactitem' id='InteractionMenu-HealPlayer' data-action='healPlayer' data-actionstring='Spieler heilen'><img src='../utils/img/healPlayer.png'></li>";
                    }
                }
        ...
        public async Task jailPlayer(ClassicPlayer player, ClassicPlayer targetPlayer)
        {
            try
            {
                if (player == null || !player.Exists || player.CharacterId <= 0 || targetPlayer == null || !targetPlayer.Exists || targetPlayer.CharacterId <= 0 || !ServerFactions.IsCharacterInAnyFaction(player.CharacterId) || !ServerFactions.IsCharacterInFactionDuty(player.CharacterId) || (ServerFactions.GetCharacterFactionId(player.CharacterId) != 1 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 3 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 8) || Characters.IsCharacterInJail(targetPlayer.CharacterId)) return;
        ...
        public async Task LockVehicle(IPlayer player, IVehicle veh)
        {
            ...
            if (ServerVehicles.GetVehicleFactionId(veh) != 0 && vehPlate.Contains(ServerFactions.GetFactionShortName(ServerVehicles.GetVehicleFactionId(veh))))
            {
                string factionPlate = ServerFactions.GetFactionShortName(ServerVehicles.GetVehicleFactionId(veh));

                if (ServerFactions.GetCharacterFactionId(charId) != ServerVehicles.GetVehicleFactionId2((int)vehID))
                {
        ...
        //    if (Serv { HUDHandler.SendNotification(player, 3, 5000, "Dieses Fahrzeug hat einen Motorschaden."); return; }
        ...
        public async Task ViewVehicleTrunk(ClassicPlayer player, ClassicVehicle veh)
        {
            try
            {
                ...
                if (veh.TrunkOccupied)
                {
                    HUDHandler.SendNotification(player, 4, 5000, "Dieser Kofferraum wird bereits verwendet!");
                    return;
                }
                ...
                veh.TrunkOccupied = true;
                player.OpenTrunkId = veh.VehicleId;
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
...

        {
            ...
            try
            {
                ...
                if (hasTargetHandcuffs)
                {
                    ...
                    if (ServerFactions.GetCharacterFactionId(charId) != 1 && ServerFactions.GetCharacterFactionId(charId) != 3) return; //LSPD FIB                    InventoryHandler.StopAnimation(targetPlayer, "mp_arresting", "sprint");
                ...
                else
                {
                    ...
                    if (ServerFactions.GetCharacterFactionId(charId) != 1 && ServerFactions.GetCharacterFactionId(charId) != 3) return; //LSPD FIB                    if (CharactersInventory.ExistCharacterItem(charId, "Handschellen", "inventory") && CharactersInventory.GetCharacterItemAmount(charId, "Handschellen", "inventory") > 0) { CharactersInventory.RemoveCharacterItemAmount(charId, "Handschellen", 1, "inventory"); }
                    ...
                    Characters.SetCharacterCurrentFunkFrequence(targetCharId, null);
