...
import { Raycast, setClothes, setTattoo, clearTattoos, setCorrectTattoos } from './utilities.js';
...
alt.onServer("Client:HUD:CreateCEF", (armor, health, hunger, thirst, currentmoney, currentmoneyblack) => {
    if (hudBrowser == null) {
        ...
        hudBrowser.on("Client:Farming:StartProcessing", (neededItem, producedItem, neededItemAmount, producedItemAmount, duration, neededItemTWO, neededItemTHREE, neededItemTWOAmount, neededItemTHREEAmount) => {
            ...
            } else if (neededItemTHREE == null) {
                alt.emitServer("Server:Farming:StartProcessing", neededItem.itemName, producedItem.itemName, neededItemAmount, producedItemAmount, duration, neededItemTWO.itemName, "none", neededItemTWOAmount, 0);
            } else {
...
alt.on('keydown', (key) => {
    if (key == 'X'.charCodeAt(0) && hudBrowser != null) {
    ...
    } else if (key == 'M'.charCodeAt(0) && hudBrowser != null) {
...
function InterActionMenuDoAction(type, action) {
    if (selectedRaycastId != null && selectedRaycastId != 0 && type != "none") {
        ...
        if (type == "vehicle") {
            ...
            } else if (action == "playerStabilisieren") {
                alt.emitServer("Server:Raycast:SpielerStabilisieren", playerRC);
            }
            playerRC = null;
        }
